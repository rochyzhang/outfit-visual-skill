...
assert.match(section(sk04Prompt, "04 COMPOSITION"), /display prop, styling support, flat display structure, and visual organizer/);
assert.match(section(sk04Prompt, "04 COMPOSITION"), /not a hidden torso, pelvis, thighs, legs, seated body, standing body, or mannequin/);
assert.match(section(sk04Prompt, "04 COMPOSITION"), /human-referential styling arrangement by garment placement only/);
assert.match(section(sk04Prompt, "04 COMPOSITION"), /loosely suggests torso and lower-body regions/);
assert.match(section(sk04Prompt, "04 COMPOSITION"), /Tops or inner tops may rest or drape over the chair while staying visibly empty/);
assert.match(section(sk04Prompt, "04 COMPOSITION"), /no chest expansion, torso cavity, stretched shoulder anatomy, or worn-body tension/);
...
assert.match(section(sk04Prompt, "04 COMPOSITION"), /waistband resting on the seat, chair edge, or under the top area/);
assert.match(section(sk04Prompt, "04 COMPOSITION"), /folds created by fabric weight, denim stiffness, garment cut, chair contact, gravity, and natural bunching/);
assert.match(section(sk04Prompt, "04 COMPOSITION"), /must not form inflated thighs, hidden knee shapes, calf volume, rounded leg tubes, sitting-leg anatomy, or a seated-body pose/);
assert.match(section(sk04Prompt, "04 COMPOSITION"), /Bags may hang from or rest against the chair as product styling/);
assert.match(section(sk04Prompt, "04 COMPOSITION"), /not as if carried by an invisible person/);
assert.match(section(sk04Prompt, "04 COMPOSITION"), /Shoes should be placed near the chair or lower composition area as clean product placement/);
assert.match(section(sk04Prompt, "04 COMPOSITION"), /not positioned as invisible feet or forced into SK06 standing-pose logic/);
...
assert.match(section(sk04Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /human-referential styling arrangement through chair-supported garment placement only/);
...
assert.match(section(sk04Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /no chest expansion, torso cavity, stretched shoulder anatomy, or worn-body tension/);
assert.match(section(sk04Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /Do not inflate trousers with invisible thigh, knee, or calf anatomy/);
assert.match(section(sk04Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /no rounded leg tubes, sitting-leg anatomy, seated-body pose encoded into trousers/);
assert.match(section(sk04Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /Natural 3D fabric folds from gravity, denim stiffness, garment cut, chair contact, and bunching are allowed/);
...
assert.match(section(sk06Prompt, "04 COMPOSITION"), /flat staged outfit with a clear standing human-pose silhouette/);
...
assert.match(section(sk06Prompt, "04 COMPOSITION"), /stronger overall body axis/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /readable shoulder area/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /readable torso/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /clearer shoulder-to-hip relationship/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /visible torso axis/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /controlled hip shift/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /obvious leg direction/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /slight body turn/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /natural fashion-pose energy/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /Human-shape feeling comes from flat product placement and pose silhouette only/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /All products remain laid out, staged, flat, empty, and product-like/);
...
assert.match(section(sk06Prompt, "04 COMPOSITION"), /pose stronger than a simple straight flat lay/);
...
assert.match(section(sk06Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /trousers directly below as lower body/);
assert.match(section(sk06Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /shoes as the coordinated end of the implied legs/);
...
assert.match(section(sk06Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /shoulder, torso, hip, and leg direction/);
assert.match(section(sk06Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /shoe toes should point in the same general direction/);
assert.match(section(sk06Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /shoes should visually continue the trouser-leg directions/);
