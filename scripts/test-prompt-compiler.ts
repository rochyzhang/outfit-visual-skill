...
assert.match(section(sk03Prompt, "04 COMPOSITION"), /approved look breakdown layout/);
assert.match(section(sk03Prompt, "04 COMPOSITION"), /one primary styled Look plus separate product\/component breakdown/);
assert.match(section(sk03Prompt, "04 COMPOSITION"), /If recommended products such as a hat or bag are present, reflow and redistribute the breakdown layout/);
assert.match(section(sk03Prompt, "04 COMPOSITION"), /do not crowd, overlap, cover each other, or squeeze the main Look/);
assert.match(section(sk03Prompt, "04 COMPOSITION"), /Keep SK03 distinct from SK01\/SK02 flat-lay label systems, SK05 Japanese catalog pages, and SK06 hand-drawn Korean street callouts/);
assert.match(section(sk03Prompt, "13 GRAPHIC OVERLAY"), /approved look breakdown typography only/);
assert.match(section(sk03Prompt, "13 GRAPHIC OVERLAY"), /clean structured editorial layout/);
assert.match(section(sk03Prompt, "13 GRAPHIC OVERLAY"), /restrained sans-serif English labels/);
assert.match(section(sk03Prompt, "13 GRAPHIC OVERLAY"), /Do not add free-floating handwritten words, slogans such as Daily Outfit/);
assert.match(section(sk03Prompt, "13 GRAPHIC OVERLAY"), /extra book props, unrelated mini images, zoom detail boxes, thumbnails, or enlarged product detail frames/);
assert.match(section(sk03Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /Do not crowd, squeeze, overlap, or cover products when optional items such as hats or bags are present/);
assert.match(section(sk03Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /Do not add free-floating handwritten words, decorative slogans, motivational phrases, Daily Outfit-style text/);
assert.match(section(sk03Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /Do not use SK01\/SK02 arrow-label styling, SK05 Japanese catalog typography, or SK06 white hand-drawn callouts in SK03/);
...
assert.match(section(sk04Prompt, "04 COMPOSITION"), /Tops or inner tops may rest over the chair back, lie over the chair back, hang over the chair edge, or be softly supported by the chair while staying visibly empty/);
...
assert.match(section(sk04Prompt, "04 COMPOSITION"), /Bags may hang naturally from or rest against the chair as independent products/);
...
assert.match(section(sk04Prompt, "04 COMPOSITION"), /Shoes and hats should stay product-like near the chair or lower composition area/);
...
assert.match(section(sk04Prompt, "04 COMPOSITION"), /Do not add secondary furniture, prop clusters, extra flat-lay mini images, detail boxes, thumbnails, books, magazines/);
...
assert.match(section(sk04Prompt, "13 GRAPHIC OVERLAY"), /only one small top information row/);
assert.match(section(sk04Prompt, "13 GRAPHIC OVERLAY"), /concise English product notes/);
assert.match(section(sk04Prompt, "13 GRAPHIC OVERLAY"), /no extra flat-lay product image row, no thumbnails, no detail boxes, no zoom frames, no book props/);
...
assert.match(section(sk04Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /Do not add flat-lay mini images, product thumbnails, detail boxes, zoom frames, books, magazines/);
...
assert.match(section(sk05Prompt, "04 COMPOSITION"), /Typography must feel like a Japanese catalog\/magazine page, not hand-drawn callouts/);
assert.match(section(sk05Prompt, "04 COMPOSITION"), /Do not create a vertical column of multiple product zoom\/detail boxes/);
assert.match(section(sk05Prompt, "04 COMPOSITION"), /extra book props, unrelated decorative mini photos, or extra slogans/);
...
assert.match(section(sk05Prompt, "13 GRAPHIC OVERLAY"), /product identification style should feel like the approved Japanese catalog reference, not handwritten callouts/);
assert.match(section(sk05Prompt, "13 GRAPHIC OVERLAY"), /large blocks of Japanese/);
assert.match(section(sk05Prompt, "13 GRAPHIC OVERLAY"), /Do not create hand-drawn text, white sketch arrows, SK06-style handwritten labels/);
assert.match(section(sk05Prompt, "13 GRAPHIC OVERLAY"), /extra slogans, book props/);
...
assert.match(section(sk05Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /Do not use hand-drawn typography, white sketch arrows, SK06 handwritten callouts/);
assert.match(section(sk05Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /book props, unrelated decorative objects, detail boxes, thumbnails, or close-up frames/);
...
assert.match(section(sk06Prompt, "04 COMPOSITION"), /not as SK05 magazine typography/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /top or outer and any hat sit at the upper-body\/head position/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /bottom is placed directly below with a believable waist-to-hem relationship/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /shoes sit at the bottom as the end of the outfit/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /both shoe toes must point left/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /if a hat is present, its brim or directional front should also face left/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /visually continue the trouser-leg directions/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /bag sits as an accessory relative to the upper-body or shoulder area/i);
...
assert.match(section(sk06Prompt, "04 COMPOSITION"), /left-facing shoe\/hat direction/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /Pose feeling must not come from body volume/);
assert.match(section(sk06Prompt, "04 COMPOSITION"), /Top, hat, trousers, bag, and shoes must remain laid out, staged, flat, empty, unworn, non-inflated, and product-like/);
...
assert.match(section(sk06Prompt, "04 COMPOSITION"), /keep shoe direction coordinated left/);
...
assert.doesNotMatch(section(sk06Prompt, "04 COMPOSITION"), /invisible person in motion/);
assert.doesNotMatch(section(sk06Prompt, "04 COMPOSITION"), /arms and legs implied through clothes/);
...
assert.match(section(sk06Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /both shoe toes should point left, and any hat brim or directional hat front should face the same leftward direction/);
...
assert.match(section(sk06Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /Do not use SK05 Japanese magazine typography, printed catalog columns, serif magazine product descriptions/);
assert.match(section(sk06Prompt, "15 NEGATIVE / PROHIBITED BEHAVIOR"), /detail boxes, thumbnails, book props, decorative slogans/);
