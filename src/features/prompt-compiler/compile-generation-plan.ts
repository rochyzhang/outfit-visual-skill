...
function buildCompositionInstruction(config: GenerationConfig) {
  ...
  const skillModifier =
    config.skillOrigin?.id === "SK01_MINIMAL_FLAT_LAY"
      ...
      ? " Skill-specific modifier for SK06: use C04 only as a base for a flat arranged human-shape outfit editorial, not as SK02's clean separated studio flat lay and not as a product breakdown. Preserve one coherent flat laid-out Look relationship: top or outer above bottom, bottom aligned to the leg area, shoes near the implied feet with both toes pointing in the same general direction, optional hat or glasses near the head position, bag at the shoulder or body side, and accessories associated with their natural styling positions. Human-shape feeling comes from flat placement and silhouette only: top and trousers align into a recognizable outfit shape while all products remain laid flat or staged on the surface. The garments are not worn, do not contain an internal body, and must not look like a standing invisible model. Treat the complete outfit as the primary composition unit before scaling individual products, maintain useful negative space around the complete look, and keep shoe direction consistent. Add a required callout for every major displayed product: small index number, short 1-3 word English product label derived conservatively from the supplied role or safe category, and a white hand-drawn-style leader line pointing to the correct item. Place annotations in surrounding negative space; never cover product details. Prefer flattened garment presentation, front-facing or mildly angled layout, low depth, low perspective, graphic silhouette, restrained garment volume, clean overall outline, and complete outfit readability."
      : "";
...
function buildSkillSceneModifier(config: GenerationConfig) {
  ...
  if (config.skillOrigin?.id === "SK04_PROP_STYLING") {
    return " Skill-specific scene fidelity for SK04: keep prop-styling identity with one visually dominant hero chair or simple prop, and restore the approved clean premium grey-white studio wall/floor. Prefer a simple light neutral backdrop, clean grey-white wall and floor, plain empty studio interior, soft diffused daylight, generous negative space, restrained premium materials, and one chair as the spatial anchor. Tiny styling details are acceptable only if they do not read as additional furniture. Avoid rough cement room feeling, industrial concrete-wall dominance, shelves, cabinets, side tables, extra pedestals unless absolutely required, multiple competing furniture pieces, decorative prop collections, cluttered tabletop styling, excessive lifestyle objects, complex room decoration, decorative architecture, busy architecture, warm yellow room tone, orange cast, overly cozy mood, or prop-heavy storytelling.";
  }
