...
function buildCompositionInstruction(config: GenerationConfig) {
  ...
  const skillModifier =
    config.skillOrigin?.id === "SK01_MINIMAL_FLAT_LAY"
      ...
      ? " Skill-specific modifier for SK03: use only the approved SK03 presentation reference as the visual target. Preserve a main look plus separated item breakdown structure. The main look may be model-based, but it must avoid a clear visible face and recognizable facial features; prefer crop below chin, head out of frame, or turned/back/side-back composition without a clear side face so the outfit, bag, trousers, and shoes remain the focus. Keep the main look present but not dominant over the item board. Build a direct fashion item breakdown / look guide / gift guide hierarchy with separated product zones, numbered items such as 01 / 02 / 03 / 04, short English product titles, and at most one very short descriptor line per item. Keep the information hierarchy clean and scannable; do not use long catalog paragraphs, portrait/model-photography page treatment, poster-style branding, SK05 catalog page pacing, or SK06 Korean street poster language."
      ...
      ? " Skill-specific modifier for SK05: use only the approved SK05 Japanese Catalog presentation reference as the visual target. Preserve the Japanese lifestyle catalog visual language: calm, quiet, soft, restrained, product-led catalog composition, gentle editorial spacing, warm-neutral paper-like mood, and simple labels with short supporting copy. Keep the existing calm catalog page structure and product arrangement logic; fix visible text by keeping all newly generated text English-only. Do not redesign SK05 merely to contrast with SK03, and do not pull visual language from SK03 or SK06."
      : config.skillOrigin?.id === "SK06_KOREAN_STREET_EDITORIAL"
      ? " Skill-specific modifier for SK06: use only the approved SK06 Korean Street Editorial presentation reference as the visual target. Restore the muted sage / dusty green flat 2D human-silhouette Korean independent-brand poster direction with a human-shaped outfit. Preserve one coherent head-to-toe outfit relationship: top or outer above bottom, bottom connected to the leg area, shoes near the implied feet, optional hat near the head position, optional glasses near the face/head position, bag at the shoulder/body side, and accessories associated with their natural styling positions. Flatness refers to depth, not incorrect scale: keep believable full-outfit human proportions while staying visually flat, graphic, clothing-only, and not volumetric. Treat the complete outfit as the primary poster subject before scaling individual products. Add white hand-drawn-style callouts for every major displayed product: small index number, short 1-3 word English product label derived conservatively from the supplied role or safe category, and a thin leader line pointing to the correct item. Allow a few minor supporting props only if they strengthen the independent-brand poster setup and remain secondary. Do not redesign SK06 relative to SK03; preserve its own flat human-shaped Korean independent-brand poster identity."
      : "";
...
function buildSkillSceneModifier(config: GenerationConfig) {
  ...
  if (config.skillOrigin?.id === "SK05_JAPANESE_CATALOG") {
    return " Skill-specific scene fidelity for SK05: use only the SK05 approved reference direction. Keep a quiet warm off-white paper-like catalog environment with soft lifestyle-catalog calm, gentle whitespace, low visual pressure, and a clean editorial page feeling. Preserve Japanese catalog visual language and calm product-led layout; the scene should support that identity without generating Japanese text, fake Japanese glyphs, poster graphics, Korean street mood, or SK03-style breakdown-board structure.";
  }

  if (config.skillOrigin?.id === "SK06_KOREAN_STREET_EDITORIAL") {
    return " Skill-specific scene fidelity for SK06: use only the SK06 approved reference direction. Keep a muted sage or dusty green poster background with clean independent-brand setup, graphic negative space, and controlled poster balance. The background should feel like a Korean independent-brand editorial poster, never like a real street-photo location, warm off-white Japanese catalog page, clean separated studio flat lay, or product breakdown board.";
  }
...
function buildSkillGraphicModifier(config: GenerationConfig) {
  ...
  if (config.skillOrigin?.id === "SK05_JAPANESE_CATALOG") {
    return "Skill-specific graphic for SK05: keep Japanese catalog visual language through quiet English-only typography, small English headings, short labels, brief notes, precise alignment, gentle whitespace, and a soft lifestyle catalog page rhythm. All newly generated visible text must be English. Preserve the calm catalog layout direction; do not add numbered item-board hierarchy just to differentiate from SK03. Do not use Japanese characters, kana, kanji, fake Japanese glyphs, pseudo-Japanese marks, Japanese-style gibberish, Hangul, Korean text, poster slogans, or dense breakdown numbering.";
  }

  if (config.skillOrigin?.id === "SK06_KOREAN_STREET_EDITORIAL") {
    return "Skill-specific graphic for SK06: restore the Korean independent-brand poster structure from SK06's own approved reference: a generic fictional logo-like title at the top, a short setup or look title near the upper or middle area, white hand-drawn-style product callouts with thin leader lines, and a very short poster-style tagline near the bottom. Keep the annotations graphic and handwritten in feel while preserving a flat outfit silhouette. Keep all visible text English-only, generic, and fictional; do not use real brand names, Hangul, Korean text, Japanese text, long copy, catalog paragraphs, or breakdown-board captions.";
  }
...
function buildProhibitedBehavior(config: GenerationConfig) {
  ...
  if (config.skillOrigin?.id === "SK03_LOOK_BREAKDOWN") {
    ...
    prohibited.push("Do not use SK05 or SK06 references, layout logic, color systems, or typography as visual anchors for SK03.");
  }
  ...
  if (config.skillOrigin?.id === "SK05_JAPANESE_CATALOG") {
    ...
    prohibited.push("Do not redesign SK05 around SK03; do not use SK03 as the style anchor, do not add SK03 numbered item-board hierarchy, and do not turn the calm catalog page into a gift guide or breakdown board.");
    prohibited.push("Do not turn SK05 into a Korean independent-brand poster, streetwear poster, strong campaign page, or generic heavy model portrait.");
    prohibited.push("Do not use large poster slogans, loud branding, dense product board structure, long catalog paragraphs, white hand-drawn Korean callouts, or muted-sage poster treatment.");
  }

  if (config.skillOrigin?.id === "SK06_KOREAN_STREET_EDITORIAL") {
    prohibited.push("Do not show a real human, visible face, visible skin, mannequin realism, strong 3D invisible-person body, walking-body anatomy, street-photo location, portrait, or model photography.");
    prohibited.push("Do not redesign SK06 around SK03; do not use SK03 as the style anchor, and do not turn SK06 into an item breakdown board, gift guide, product catalog page, or quiet Japanese catalog layout.");
    prohibited.push("Do not turn SK06 into SK02's clean separated studio flat lay, SK01's natural concrete flat lay, or any real-person street photo.");
